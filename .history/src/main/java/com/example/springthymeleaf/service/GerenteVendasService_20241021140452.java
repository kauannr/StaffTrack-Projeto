...
public class GerenteVendasService {
    ...
    public Page<GerenteVendas> findByNamePage(String nome, int page, int size, String sortedBy) {
        Pageable pageable = PageRequest.of(page, size, Sort.by(sortedBy));
        return gerenteVendasRepository.findByNome(nome, pageable);
    }

    public Page<GerenteVendas> findBySobrenomePage(String sobrenome, int page, int size, String sortedBy) {
        Pageable pageable = PageRequest.of(page, size, Sort.by(sortedBy));
        return gerenteVendasRepository.findBySobrenome(sobrenome, pageable);
    }

    public Page<GerenteVendas> findByIdPage(String sobrenome, int page, int size, String sortedBy) {
        Pageable pageable = PageRequest.of(page, size, Sort.by(sortedBy));
        return gerenteVendasRepository.findById(sobrenome, pageable);
    }
}
