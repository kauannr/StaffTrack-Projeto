...
public class GerenteVendasController {
    ...
    public ModelAndView salvar(@Valid GerenteVendas gerenteVendas, BindingResult bindingResult) {
        ...
        contrato.get().setPessoa(gerenteVendas);
        ...
        Set<ConstraintViolation<GerenteVendas>> violacao = validator.validate(gerenteVendas);
        if(!violacao.isEmpty()){
            List<String> msgErro = new 
            for (ConstraintViolation<GerenteVendas> constraintViolation : violacao) {
