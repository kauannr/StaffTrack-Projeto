...
public class VendedorController {
    ...
    public ModelAndView pesquisarVendas(@RequestParam(required = false) Long idVenda,
        ...
        ModelAndView modelAndView = new ModelAndView("redirect:/pesquisarvendas");

        if (idVenda != null) {
            Page<Vendas> listaVendas = new PageImpl<>(Collections.singletonList(vendasService.findById(idVenda).orElse(null)));
            redirectAttributes.addFlashAttribute("listaVendas", listaVendas);
        } else if (valorVenda != null) {
            Page<Vendas> listaVendas = vendasService.findByValorVenda(valorVenda, PageRequest.of(page, size));
            redirectAttributes.addFlashAttribute("listaVendas", listaVendas);
        } else {
            Page<Vendas> listaVendas = vendasService.findAll(PageRequest.of(page, size, Sort.by("id")));
            redirectAttributes.addFlashAttribute("listaVendas", listaVendas);
        }
    
        redirectAttributes.addFlashAttribute("objVenda", new Vendas());
        redirectAttributes.addFlashAttribute("objVendedor", new Vendedor());
